...
class MaskedFaceRecog:
    def __init__(self):
        self.flag_mask = 0
        self.identity_check_succeeded = False
        # 加载检测模型
        ...
        while True:
            ...
            for index,bbox in enumerate(bboxes):
                ...
                if min_dist < threshold:
                    ...
                    if self.flag_mask==1:
                        self.identity_check_succeeded = True # 身份校验成功
                    elif self.flag_mask==0:
                        cv2.putText(frame,"Please wear a mask!",(50,50),cv2.FONT_ITALIC,1,(0,0,255),2)
                else:
                    # 未识别到，加载未知
                    name_overlay = self.name_png_list['未知']
                    cv2.putText(frame,"Validation failed...",(50,50),cv2.FONT_ITALIC,1,(0,0,255),2)
                # √和×标志
            ...
            if self.identity_check_succeeded:
                # 身份校验成功
                # cv2.putText(frame,"Verification passed!",(50,50),cv2.FONT_ITALIC,1,(0,255,0),2)
                # cv2.imshow('facenet_wearMask',frame)
                # time.sleep(3)
                cap.release()
